...
public class DaoDocente {
    ...
    public List<Docente> listarDocentes() {
        List<Docente> docentes = new ArrayList<>();
        String sql = "SELECT idDocente, matricula, nombre, apellido, contraseña, estatus, (AVG(CalificacionDocente)*2) AS Calificacion FROM Docentes JOIN  Historial_asesorias ON Docentes.idDocente = Historial_asesorias.id_Docente group by idDocente, matricula, nombre, apellido, contraseña, estatus";
        try (Connection connection = DriverManager.getConnection(JDBC_URL, JDBC_USER, JDBC_PASSWORD);
             PreparedStatement stmt = connection.prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {
